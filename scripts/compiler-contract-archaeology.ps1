...
function CompilerAreaName {
    ...
    if ($Name -match '^(COpt|OPT|OPTF_|OPTf_)') { return "optimizer-options" }
    ...
    if ($Name -match '^(CCmp|Cmp|CMP)') { return "compiler-control" }
    if ($Name -match '^(CHash|Hash)') { return "hash-symbol-table" }
    if ($Name -match '^(CCode|Code|CStreamBlk)') { return "code-stream" }
    return "other"
}
